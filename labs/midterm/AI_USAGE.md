...
| 14:54 | B2.3 | ถามวิธีทำให้ช่องค้นหาทำงานร่วมกับตัวกรองสถานะเดิม | ใช้คำแนะนำให้สร้างเงื่อนไข `matchesStatus` และ `matchesSearch` แล้วรวมทั้งสองเงื่อนไขด้วย `&&` ใน `filteredRequests` | เลือกตัวกรอง `completed` และพิมพ์คำว่า `ห้อง` แล้วตรวจว่ารายการเหลือเฉพาะ REQ-003 ขณะที่ Summary ยังคงแสดง 5/3/1/1 |
| 15:05 | B2.4 | เพิ่ม Conditional Rendering สำหรับกรณีค้นหาแล้วไม่พบรายการ โดยตรวจสอบว่า `searchTerm` มีข้อความและ `filteredRequests.length === 0` จากนั้นแสดงข้อความ “ไม่พบคำร้องที่ตรงกับการค้นหา” พร้อมกำหนด `role="status"` เพื่อรองรับ Accessibility ส่วน Summary ยังคำนวณจาก `requests` ทั้งหมด ไม่ได้คำนวณจากรายการที่ผ่านการค้นหา ทดสอบด้วยคำว่า `zzz` แล้วระบบแสดงข้อความไม่พบข้อมูล ขณะที่ Summary ยังคงเป็น 5/3/1/1 |
